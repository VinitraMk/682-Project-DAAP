...
def foundation_defense(attacked_img_folder, save_folder, args):
    ...
    for img, label, img_name in tqdm.tqdm(dataset, desc="Defending"):
        ...
        masks, boxes, phrases, logits = model.predict(img, text_prompt)


        try:
            mask = masks[0]
            masked_img = 255*to_tensor(img) * (~mask)
        except IndexError:
            masked_img = 255*to_tensor(img)
        ...
        write_png(masked_img.to(torch.uint8), img_save_path)
        cv2.imwrite(mask_save_path, 255*mask.numpy())
...
if __name__ == "__main__":
    ...
    model = LangSAM()
    image_pil = Image.open("../sample.png").convert("RGB")
    text_prompt = "adversarial patch"
    ...
    cv2.imwrite("sam_mask.png", masks)